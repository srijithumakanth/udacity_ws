...
#define EPS_DECAY 250 //200 just for the arm; 250 for the gripper
...
#define LEARNING_RATE 0.9f //0.9 for the arm; 1.0 for the gripper.
#define REPLAY_MEMORY 10000
#define BATCH_SIZE 128 //8 64 for the arm
...
bool ArmPlugin::createAgent()
{
	...
	// Creating DQN agent and passing all the hyperparameters
...
{
	// Reward for any part of arm given only when GRIPPER == 0
	rewardHistory = GRIPPER ? REWARD_LOSS*REWARD_LOSS*REWARD_LOSS : REWARD_WIN;
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		// Check gripper contact the ground.
